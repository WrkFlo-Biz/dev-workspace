...
LOG_RETENTION_WEEKS="${DWS_LOG_RETENTION_WEEKS:-4}"
...
build_jobs() {
  ...
  if [ -n "${DWS_LOG_ROTATE_SCRIPT:-}" ]; then
    ...
    ROTATE_JOB="\"${ROTATE_SCRIPT}\" --keep-weeks ${LOG_RETENTION_WEEKS}"
...
validate_config() {
  is_int "$LOG_RETENTION_WEEKS" || die "DWS_LOG_RETENTION_WEEKS must be an integer"
  [ "$LOG_RETENTION_WEEKS" -ge 1 ] || die "DWS_LOG_RETENTION_WEEKS must be at least 1"
