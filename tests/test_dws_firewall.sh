...
EOF
  ...
  assert_contains "${output}" 'tailscale subnet allowlist: tcp access is restricted to 100.64.0.0/10'
...
test_ufw_verify_fails_when_ssh_is_public() {
  local output
...
EOF
  ...
  assert_contains "${output}" 'verification failed: unexpected public tcp/22 rule detected'
}
