...
| Task monitor is not boot-managed | `monitor` runs only in `tmux`; no `task-monitor.service` exists | worker automation does not recover automatically after reboot | add managed startup for session-init plus task-monitor |
...
| Logging is fragmented under `/tmp` | monitor, cron, cleanup, health, phone, and helper logs are scattered | poor triage and weak retention | centralize log tree and update writers |
...
| Runtime docs lag live behavior | older docs still reference `planner` and manual monitor restarts | operator confusion in incidents | refresh docs from runtime truth before broader changes |
